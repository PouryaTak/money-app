...
export const groupTransactionsByTypeCategory = (
  ...
  const findCategory = (category: string) => categories[type].find((i: any) => i.key == category);
...
export const convertChartData = (data: Array<CategorizedTransaction>): Array<{ x: string; y: number }> => {
  return data.map((item: CategorizedTransaction) => ({ x: item.category, y: item.value, color:item.color }));
};
